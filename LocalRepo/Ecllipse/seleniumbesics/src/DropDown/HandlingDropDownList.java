...
public class HandlingDropDownList {
	...
	//	List actualList=new ArrayList();
		...
		for(int i=0;i<daysCount.size();i++) {
			...
			List<WebElement>monthCount=driver.findElements(By.cssSelector("select[placeholder='Month']"));
			
			
			System.out.println("getting the month count:"+monthCount.size());
			for(int j=0;j<monthCount.size();j++) {
			System.out.println("Getting All the Months Name:"+monthCount.get(i).getText());
